...
fn test_secrets_create_with_one_field() -> Result<(), Error> {
    let mut p = spawn("./target/debug/zero-cli secrets create --id dc1c", Some(15000))?;
...
fn test_secrets_delete() -> Result<(), Error> {
    let mut p = spawn("./target/debug/zero-cli secrets delete --id fc2d", Some(15000))?;
...
fn test_secrets_edit() -> Result<(), Error> {
    let mut p = spawn("./target/debug/zero-cli secrets edit --id fc1d5", Some(15000))?;
...
fn test_secrets_list() -> Result<(), Error> {
    let mut p = spawn("./target/debug/zero-cli secrets list --id dc1c", Some(15000))?;
    p.exp_string("#fc3d")?;
...
fn test_secrets_share() -> Result<(), Error> {
    let mut p = spawn("./target/debug/zero-cli secrets share --id fc4d", Some(15000))?;
...
fn test_secrets_view() -> Result<(), Error> {
    let mut p = spawn("./target/debug/zero-cli secrets view --id fc4d", Some(15000))?;
    p.exp_string("URL")?;
    p.exp_string("Name")?;
    p.exp_string("Vendor")?;
    p.exp_string("Fields")?;
